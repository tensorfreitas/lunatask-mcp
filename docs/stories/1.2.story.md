...
## Tasks / Subtasks
- [ ] Task 1: Create main application entry point (AC: 1)
  - [ ] Subtask 1.1: Create `src/lunatask_mcp/main.py` according to source tree architecture
  - [ ] Subtask 1.2: Implement CoreServer class as the main application runner
  - [ ] Subtask 1.3: Configure FastMCP application instance
- [ ] Task 2: Implement basic FastMCP server (AC: 2, 3, 14)
  - [ ] Subtask 2.1: Initialize FastMCP instance with stdio transport
  - [ ] Subtask 2.2: Configure server to listen on stdio by default
  - [ ] Subtask 2.3: Use the standard asyncio loop runner for the async main entry point
  - [ ] Subtask 2.4: Initialize with explicit MCP protocol version `2025-06-18` and verify negotiation
...
- Entry point script configured in `pyproject.toml` as `lunatask-mcp = "lunatask_mcp:main"`
