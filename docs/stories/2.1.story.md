...
3. On success, returns a JSON-serializable list of task objects conforming to the TaskResponse schema; gracefully handles the absence of encrypted fields (e.g., `name`, `note`).
...
- [x] Task 1: Extend LunaTaskClient with get_tasks method (AC: 2, 3, 4, 6)
  ...
  - [x] Subtask 1.5: Ensure encrypted fields (name, note) absence is handled correctly
...
- Key fields: `id`, `area_id`, `status`, `priority`, `due_date`, `created_at`, `updated_at`, `source` [Source: architecture/4-data-models.md#taskresponse-response-model]
...
- **Task 1.1 (Subtask 1.1)**: Successfully implemented `async get_tasks() -> List[TaskResponse]` method in `LunaTaskClient`
  ...
  - Handles encrypted field absence gracefully (name/note not present in E2E encrypted responses)
...
- **Task 1.5 (Subtask 1.5)**: Verified encrypted fields (name, note) absence is handled correctly
  - TaskResponse model in `models.py` intentionally excludes `name` and `note` fields due to E2E encryption
...
- **Task 2.3 (Subtask 2.3)**: Successfully connected resource to `LunaTaskClient.get_tasks()` method
  ...
  - Maintains all TaskResponse fields: id, area_id, status, priority, due_date, created_at, updated_at, source
...
- **Task 2.4 (Subtask 2.4)**: Verified JSON formatting for MCP client consumption is implemented correctly
  ...
  - Structured resource response with metadata including total_count and encrypted field note
