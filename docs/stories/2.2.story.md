...
## Tasks / Subtasks
- [ ] Task 1: Extend LunaTaskClient with get_task method using TDD (AC: 1, 3, 4, 5)
  - [ ] Subtask 1.1: Write tests for `get_task(task_id: str) -> TaskResponse` method (success, not found, encrypted fields, errors)
  - [ ] Subtask 1.2: Watch tests fail - implement async `get_task(task_id: str) -> TaskResponse` method in LunaTaskClient
  - [ ] Subtask 1.3: Add authenticated GET request to `/v1/tasks/{task_id}` endpoint with rate limiting
  - [ ] Subtask 1.4: Parse response JSON into TaskResponse model instance
  - [ ] Subtask 1.5: Handle task not found (404) with specific TaskNotFoundError
  - [ ] Subtask 1.6: Refactor - ensure all tests pass and code is clean
- [ ] Task 2: Enhance TaskTools with single task resource using TDD (AC: 1, 2, 3, 4, 5)
  - [ ] Subtask 2.1: Write tests for MCP resource template `lunatask://tasks/{task_id}` (parameter extraction, success, errors)
  - [ ] Subtask 2.2: Watch tests fail - implement MCP resource template with URI `lunatask://tasks/{task_id}` using @mcp.resource decorator
  - [ ] Subtask 2.3: Connect resource to `LunaTaskClient.get_task(task_id)` method
  - [ ] Subtask 2.4: Format single task response as JSON for MCP client consumption
  - [ ] Subtask 2.5: Implement proper error handling including TaskNotFoundError translation
  - [ ] Subtask 2.6: Refactor - ensure resource template registration and all tests pass
