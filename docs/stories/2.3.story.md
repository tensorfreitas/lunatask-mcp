...
## Tasks / Subtasks
- [ ] Task 1: Extend LunaTaskClient with create_task method using TDD (AC: 1, 3)
  - [ ] Subtask 1.1: Write tests for `create_task(task_data: TaskCreate) -> TaskResponse` method (success, validation errors, auth errors, rate limits)
  - [ ] Subtask 1.2: Watch tests fail - implement async `create_task(task_data: TaskCreate) -> TaskResponse` method in LunaTaskClient
  - [ ] Subtask 1.3: Add authenticated POST request to `/v1/tasks` endpoint with JSON payload and rate limiting
  - [ ] Subtask 1.4: Parse response JSON into TaskResponse model instance (note: returned task will have assigned ID)
  - [ ] Subtask 1.5: Handle validation errors (422) with LunaTaskValidationError
  - [ ] Subtask 1.6: Handle subscription limit errors (402) with LunaTaskSubscriptionRequiredError
  - [ ] Subtask 1.7: Refactor - ensure all tests pass and code is clean
