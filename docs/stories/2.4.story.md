...
2. The tool accepts a task ID and modifiable fields (name, note, area_id, status, priority, scheduled_on, source).
...
- [x] Task 2: Implement update_task MCP tool in TaskTools using TDD (AC: 1, 2, 3)
  ...
  - [x] Subtask 2.4: Parse and validate scheduled_on YYYY-MM-DD string to date
...
- [x] Task 3: End-to-End validation testing (AC: 1, 2, 3)
  ...
  - [x] Subtask 3.5: Test date format handling for scheduled_on parameter
...
- All fields optional to support partial updates: `name`, `note`, `area_id`, `status`, `priority`, `scheduled_on`, `source` [Source: Research plan]
- Date fields use YYYY-MM-DD format with Pydantic parsing/validation [Source: architecture/4-data-models.md]
...
- scheduled_on YYYY-MM-DD string parsing and validation
- Invalid scheduled_on format raises validation error
...
- Test date format handling for scheduled_on (YYYY-MM-DD format)
...
- Ensure proper JSON serialization of date fields
...
- Tool accepts required task_id parameter and optional update fields (name, note, area_id, status, priority, scheduled_on)
- Comprehensive parameter validation: empty task_id detection, at least one field required for update
- YYYY-MM-DD scheduled_on string parsing and validation with proper error handling
...
- Date format handling validated for YYYY-MM-DD formats with proper validation
- Invalid date formats properly rejected with appropriate validation error messages
...
- Verification includes tool schema validation, error translation, and date format validation
